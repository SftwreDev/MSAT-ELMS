...
class Exams(models.Model):
    ...
    description = models.TextField(
        ...
        blank=True, help_text=_("a description of the exams"))
    ...
    draft = models.BooleanField(
        ...
        help_text=_("If yes, the exams is not displayed"
                    " in the exams list and can only be"
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        super(Exams, self).save(force_insert, force_update, *args, **kwargs)
...
class Progress(models.Model):
    ...
    """
    user = models.OneToOneField(settings.AUTH_USER_MODEL, verbose_name=_("User"), related_name='user_progress_exam', on_delete=models.CASCADE)
...
class SittingManager(models.Manager):
    ...
    def new_sitting(self, user, exams):
        if exams.random_order is True:
            question_set = exams.question_set.all() \
                                            .select_subclasses() \
                                            .order_by('?')
        else:
            question_set = exams.question_set.all() \
        ...
        if len(question_set) == 0:
            raise ImproperlyConfigured('Question set for exams is empty. '
                                       'Please configure questions properly')

        if exams.max_questions and exams.max_questions < len(question_set):
            question_set = question_set[:exams.max_questions]
    ...
    def user_sitting(self, user, exams):
        if exams.single_attempt is True and self.filter(user=user,
                                                       exams=exams,
                                                       complete=True)\
                                               .exists():
            return False
        ...
        try:
            sitting = self.get(user=user, exams=exams, complete=False)
        except Sitting.DoesNotExist:
            sitting = self.new_sitting(user, exams)
        except Sitting.MultipleObjectsReturned:
            sitting = self.filter(user=user, exams=exams, complete=False)[0]
        return sitting
...
class Sitting(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_("User"), related_name='user_sitting_exam', on_delete=models.CASCADE)

    exams = models.ForeignKey(Exams, verbose_name=_("Exams"), on_delete=models.CASCADE)
    ...
    def check_if_passed(self):
        return self.get_percent_correct >= self.exams.pass_mark

    @property
    def result_message(self):
        if self.check_if_passed:
            return self.exams.success_text
        else:
            return self.exams.fail_text
    ...
    def get_questions(self, with_answers=False):
        ...
        questions = sorted(
            self.exams.question_set.filter(id__in=question_ids)
                                  .select_subclasses(),
...
class Answer(models.Model):
    question = models.ForeignKey(ExamsQuestions, verbose_name='Question', on_delete=models.CASCADE)
